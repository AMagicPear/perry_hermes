...
- Context compaction is intentionally simple: default-on, one built-in `SummaryCompactor`, provider-usage-only threshold checks, and compacted history shaped as system prompt + first user message + one LLM summary message. `CompactionStrategy` is policy only; session state and context-window accounting belong to `AgentSession` / `SessionState`.
